package backend.monolith.service
...
class EntityManager(
     ...
     */
    fun createSelect(selectFrom: SelectFromAndJoin, entityType: Class<*>, pageable: Pageable?, criteria: Criteria?): String {
        if (pageable != null) {
            if (criteria != null) {
                return createSelectImpl(selectFrom.limitOffset(pageable.pageSize.toLong(), pageable.offset).where(Conditions.just(criteria.toString())), entityType, pageable.sort)
            }
     ...
     * @return sql select statement
     */
    fun createSelect(selectFrom: SelectFromAndJoinCondition, entityType: Class<*>, pageable: Pageable?, criteria: Criteria?): String {
        if (pageable != null) {
            if (criteria != null) {
                return createSelectImpl(selectFrom.limitOffset(pageable.pageSize.toLong(), pageable.offset).where(Conditions.just(criteria.toString())), entityType, pageable.sort)
            } else {
     ...
     */
    fun updateLinkTable(table: LinkTable, entityId: Long?, referencedIds: List<Long?>?): Mono<Int> {
